...
export function getTheme({ theme, name }: { theme: 'zen-dark' | 'zen-sakura'; name: string }) {
  ...
  const onlyHighContrast = (color: string) => {
    return themes({ light_high_contrast: color, dark_high_contrast: color });
  };

  const onlyDarkHighContrast = (color: string) => {
...
function changeColorToHexAlphas(values: Record<string, any>) {
  const obj = { ...values };
  for (var keys in obj) {
